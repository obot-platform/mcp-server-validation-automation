...
When(/^User sends prompts to Obot AI chat for "([^"]*)" MCP server$/, { timeout: 15 * 60 * 1000 }, async function(serverName: string) {
  const jsonPath = path.resolve(process.cwd(), 'src', 'data', `${serverName.toLowerCase().replace(/\s+/g, '_')}.MCP.json`);
...
Then(/^All prompt results should be validated and a report generated for the selected "(.*)" MCP server$/, async function(serverName: string) {
