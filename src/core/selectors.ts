const Selectors = {
  ...
  createnewobotButton:
    "//button[contains(@class, 'button') and contains(., 'Create New Agent')]",
  editpencilIcon:
    '//*[contains(@class,"pencil h-5")]/ancestor::button[contains(@class,"group text")]',
  messageContent:
    '//div[@class="flex w-full items-center gap-4 p-2"]//textarea[@placeholder="Your message..."]',
  updateUsername: '//input[@class="bg-surface grow rounded-lg p-2"]',
  exitEditor:
    '//button[@class="group relative mr-1 flex items-center rounded-full border p-2 text-xs transition-[background-color] duration-200 border-blue bg-blue text-white md:px-4"]',
  ...
  fileuploadPopup:
    "//div[contains(@class, 'default-dialog') and @style='left: 166px; top: 12.833px;']",
  fileuploadButton:
    "(//label[contains(@class, 'button') and contains(@class, 'flex') and contains(., 'Upload')])[2]",
  filenameVerify: '//span[text()="AI_test.txt"]',
  fileUpload:
    '//label[contains(normalize-space(), "Upload")]//input[@type="file" and @accept=".pdf, .txt, .doc, .docx, .ppt, .pptx, .md, .rtf, .html, .odt, .ipynb, .json,.csv, .png, .jpg, .jpeg, .webp"]',
  confirmDeleteObotButton:
    '//h3[text()="Delete the current Obot?"]/following-sibling::button[contains(text(), "Yes") and contains(@class, "bg-red-600")]',
  clickChatbot:
    '//button[@class="icon-button group relative flex items-center gap-2 p-0 shadow-md"]',
  clickOnDiv: '//div[@id="click-catch"]',
  clickshowOutput: '//button[text()="Show Output"]',
  chatOutputElement:
    '//p[text()="Output"]/following-sibling::pre[contains(@class,"text")]',
  task: {
    expandTaskMenu:
      '//div[@class="flex items-center justify-between"]//button[@class="icon-button"][1]',
    ...
    deleteTask:
      '//strong[text()="TASK"]/../..//button[@class="button-destructive p-4"]',
    confirmDeleteTask:
      '//h3[contains(text(),"delete this task")]/following-sibling::button[contains(text(), "Yes") and contains(@class, "bg-red-600")]',
    stepsInput:
      '//textarea[@class="ghost-input border-surface2 ml-1 grow resize-none scrollbar-none"]',
    runButton:
      "(//button[contains(@class, 'bg-blue') and contains(@class, 'text-white')])",
    ...
    argumentPopup:
      '//h4[text()="Arguments"]/following-sibling::div//input[@id="param-tax calculator"]',
    argumentValue: '//input[@id="param-Enter your income"]',
    submitArgs:
      "(//button[text()='Run' and contains(@class, 'button-primary')])[2]",
    inputResult:
      "(//div[contains(@class, 'relative') and contains(@class, 'my-3')])[3]",
    inputTextHeader: '//p[contains(text(),"Here is the tax on your income")]',
    inputText:
      '//span[@class="text-sm font-semibold" and contains(text(),"Automation")]',
    searchBot: '//textarea[@id="chat"]',
    submitButton:
      '//button[@type="submit" and contains(@class, "button-colors") and contains(@class, "text-blue")]',
    triggerTypeBtn: '(//button[normalize-space(text())="on demand"])[1]',
    selectTriggerType: (option: string) =>
      `//button[normalize-space(text())='${option}']`,
    webHookURL:
      '//div[contains(@class, "bg-surface2") and contains(., "https://")]',
    runTask: '//div[contains(@class, "flex") and contains(@class, "truncate")]',
    taskResult: '//ul[@class="ml-4 flex flex-col text-xs"]//li[1]//div',
    tableResult:
      '//span[text()="Tables "]/../following-sibling::div/div/div/ul/li/button',
    tableRefresh: '//button[text()=" Refresh"]',
    TaskMenu: '//button[.//span[contains(text(), "Tasks")]]',
    sideBar: '(//button[@class="icon-button"])[1]',
    timeTypeBtn: (option: string) =>
      `(//button[normalize-space(text())='${option}'])[1]`,
  ...
  table: {
    ...
    submitInput:
      '(//button[@type="submit" and contains(@class,"button-colors")])[2]',
  },
  ...
  expandFileicon:
    '//span[text()="Files"]/parent::button[@class="flex items-center gap-2 px-5 py-2"]',
  ...
  knowledgeFileUpload:
    '//label[contains(normalize-space(), "Upload")]//input[@type="file" and @accept=".pdf, .txt, .doc, .docx"]',
  ...
  starterMsgButton:
    '//span[text()="Starter Message"]/ancestor::button[contains(@class,"button")]',
  ...
  tableInput:
    '//pre[contains(@class, "whitespace-pre-wrap")]//span[contains(@class, "text-gray-500")]',
  ...
  admin: {
    ...
    deleteAgent:
      '(//a[contains(@href, "/admin/agents/")]/following-sibling::button)[1]',
    confirmDelete: '//div[text()="Confirm"]',
    agenetDeletedMessagge:
      '//div[@data-content]//div[contains(text(), "Agent deleted")]',
    memoryDropdown:
      '//p[text()="Memory"]/ancestor::div[contains(@class, "flex")]/following-sibling::div//button',
    setAlwaysOn:
      '//div[contains(@role, "option")]/span[contains(text(),"Always On")]',
  ...
  authLink:
    '//a[contains(@class, "bg-blue") and contains(@class, "rounded-3xl") and contains(@class, "text-white")]',
  ...
  continueBtn:
    '//button[contains(text(), "I dunno")]/following-sibling::button',
  ...
  memoryIcon:
    '//button[@data-memories-btn and contains(@class, "icon-button")]',
  ...
  validateMCPTool: (option: string) =>
    `//p[normalize-space(text())="${option}"]`,
  knowledge: '//span[text()="Knowledge "]',
  selectFromSidebar: `(//span[(text())="{{select}}"])`,
  enableChatBot: '//h5[text()="Enable"]/../label',
  copyTemplates:
    '//div/a[@class="overflow-hidden text-sm text-ellipsis hover:underline"]',
  copyChatBot:
    '//div/a[@class="overflow-hidden text-ellipsis hover:underline"]',
  ...
  templatePencil:
    '(//button[contains(@class, "text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200")])[2]',
  ...
  signinHubspot:
    "//i18n-string[normalize-space()='Sign in to your HubSpot account']",
  ...
  MCP: {
    ...
    connectorbtn:
      '//p[normalize-space(text())="Connectors"]/following-sibling::button',
    mcpSearchInput:
      '//input[normalize-space(@placeholder)="Search by name..."]',
    // selectMCPServer: '//p[normalize-space(text())="WordPress"]',
    selectMCPServer: (option: string) =>
      `//p[normalize-space(text())="${option}"]/ancestor::div[contains(@class, 'flex')]/button`,
    ...
    btnClick: (option: string) =>
      `//button[normalize-space(text())="${option}"]`,
    promptInput:
      '//div[@class="plaintext-editor text-md relative w-full flex-1 grow resize-none p-2 leading-8 outline-none"]',
    ...
    messageContainer:
      "//div[contains(@class, 'flex-1') and contains(@class, 'flex-col') and contains(@class, 'justify-start') and contains(@class, 'gap-8')]",
  },
};
